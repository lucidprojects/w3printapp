...
namespace PrintInvoice
{
    public partial class MainForm
    {
        private void reprint_UpdateList(object sender, EventArgs e)
        {
            ...
            {
                dgvReprint.ColumnCount = (_reprint.FieldNames?.Length ?? 0) + 1;
            ...
            for (var i = 0; i < _reprint.PackageList.Count; i++)
            {
                ...
                dgvReprint.Rows[i].ErrorText = _reprint.PackageList[i].IsError ? _reprint.PackageList[i].ErrorText : "";
            }
        ...
        {
            var packageList = (from DataGridViewRow row in dgvReprint.SelectedRows select row.Tag as PrintPackageWrapper).ToList();
        ...
        {
            if (MessageBox.Show(this, @"Do you want to clear reprint list?", @"Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                _reprint.clear();
        ...
        {
            _reprint.SetPackageState(e.PrintInvoiceWrapper.PackageId, e.PrintInvoiceWrapper.IsLoaded ? PrintPackageWrapper.Loaded : PrintPackageWrapper.Locked, "");
        }
...
Continue printing?",
                ...
                if (dgvReprint.CurrentRow.Index == 0 || (dgvReprint.CurrentRow.Index > 0 && MessageBox.Show(@"You are going to print not from the beginning. Continue?", @"Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes))
                {
                    btReprintPrint.Enabled = false;
                            ...
                            if (package.PackageId != PackageWrapper.NullPackageId)
                            {
                                ...
                                if (package.IsPrinted)
                                {
                                    if (askRepeatedPrint)
                                    {
                                        ...
                                        fmRepeatedPrint.laMessage.Text = $@"You are going to print invoice No. {dgvReprint.Rows[i].Cells[1].Value} is marked as printed. Do you want to print it again?";
        ...
        private void btReprintLastBatches_Click(object sender, EventArgs e)
        {
            ...
            var nLoaded = _fmLastBatches.OnLoad();
